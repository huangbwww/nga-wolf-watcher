...
DEFAULT_CONFIG = nga_wolf_config.DEFAULT_CONFIG
...
def load_feishu_profiles(config: dict[str, object]) -> list[dict[str, Any]]:
    return nga_wolf_config.load_feishu_profiles(config)


def load_wechat_profiles(config: dict[str, object]) -> list[dict[str, Any]]:
    return nga_wolf_config.load_wechat_profiles(config)


def load_dingtalk_profiles(config: dict[str, object]) -> list[dict[str, Any]]:
    return nga_wolf_config.load_dingtalk_profiles(config)
...
def load_listen_rules(config: dict[str, object]) -> list[dict[str, Any]]:
    return nga_wolf_config.load_listen_rules(config)
...
def int_value(config: dict[str, object], key: str, default: int) -> int:
    return nga_wolf_config.int_value(config, key, default)


def float_value(config: dict[str, object], key: str, default: float) -> float:
    return nga_wolf_config.float_value(config, key, default)
...
class App:
    def __init__(self, root: ctk.CTk) -> None:
        ...
        self.selected_email_profile_index = 0 if self.email_profiles else -1
    ...
    def collect_config(self) -> dict[str, object]:
        ...
        if getattr(self, "email_profiles", []):
            profile = self.email_profiles[0]
            config["email_smtp_host"] = str(profile.get("smtp_host") or "smtp.gmail.com").strip()
            config["email_smtp_port"] = str(profile.get("smtp_port") or "587").strip()
            config["email_smtp_security"] = str(profile.get("smtp_security") or "starttls").strip()
            config["email_username"] = str(profile.get("username") or "").strip()
            config["email_password"] = str(profile.get("password") or "").strip()
            config["email_from"] = str(profile.get("from_email") or "").strip()
            config["email_from_name"] = str(profile.get("from_name") or "NGA Wolf Watcher").strip()
            config["email_reply_to"] = str(profile.get("reply_to") or "").strip()
        else:
            config["email_smtp_host"] = "smtp.gmail.com"
            config["email_smtp_port"] = "587"
            config["email_smtp_security"] = "starttls"
