...
- Codex model dropdown includes `gpt-6-astra` (GPT-6 Astra), alongside `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, and the retained older model choices. The default remains Sol with `high` reasoning. Astra, Sol, and Terra support `low`, `medium`, `high`, `xhigh`, `max`, and `ultra` in Codex; Luna supports through `max`; retained 5.5/5.4 models support through `xhigh`. `max` gives one task the most reasoning time, while Codex's `ultra` mode may automatically delegate work to subagents and is not available for Luna. Select Astra in AI settings or send `/model gpt-6-astra` to the bot. The model ID is verified against the [official GPT-6 Astra documentation](https://developers.openai.com/api/docs/models/gpt-6-astra); `ultra` is a Codex mode reported by the local model catalog, not a separate public API reasoning level.
