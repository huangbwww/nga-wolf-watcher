...
从 `v1.3.0` 开始，Release 增加 Linux 服务器安装方式。服务器上可以用一行命令安装 `ngawolf`，再通过终端 TUI 配置 Cookie、推送通道、NGA 用户/帖子和监听规则；安装版默认写入 systemd 服务，也可以用 `ngawolf start/stop/status/logs` 管理后台运行和日志。
