...
默认消息通道是 `飞书`，旧配置和旧用法保持不变。新版 GUI 顶部有 `消息通道` 选择：

- 选 `feishu` 时，只需要填写飞书应用配置；微信 Token 为空不会报错。
- 选 `wechat` 时，只需要填写微信 Bot 配置；飞书 App ID、App Secret、Receive ID 为空不会报错。
- NGA Cookie、监听间隔、免打扰、AI 分析配置是公共配置，会跟随当前通道发送消息。
